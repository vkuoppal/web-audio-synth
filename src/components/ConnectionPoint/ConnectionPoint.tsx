  ...
  render() {
    ...
    const connectionPointStyle = this.props.forcedPosition
      ? {
        ...
        boxShadow: "0px 2px 4px 2px rgba(0, 0, 0, 0.45), inset black 0 0px 2px, rgb(0, 140, 158) 0px 0px 5px 0px",
      }
      : {}
    return (
      // @ts-ignore
      <div className="connection-point-container" style={style} ref={this.connectionPointRef} onTouchStart={(e) => {
        if (this.props.startConnection && this.props.moveConnection) {
          const { x, y, width, height } = this.connectionPointRef.current.getBoundingClientRect();
        ...
        <div className="connection-point" style={connectionPointStyle}>
        </div>
      </div >
    );
  }
